...
def Twist_Arm_L():
    ...
    num=1
    List=['J_Arm_Neutral_L','J_ForeArm_Neutral_L','J_Hand_Neutral_L']
    for J in List:
    ...
    cmds.move(2*Scale_Guide,0,0,'Z_Hand_Eff_Twist_L',r=True,os=True,wd=True)
    Joints=['Arm','ForeArm']
    for Joint in Joints:
        if Joint=='ForeArm':
            CreateNode('multDoubleLinear','MDL_'+Joint+'_00_Twist_L')
            cmds.connectAttr('Loc_Extract_'+Joint+'_L.rotate','J_Bendy_3_Arm_L.rotate')
            ...
            cmds.connectAttr('MDL_'+Joint+'_01_Twist_L.output','J_Bendy_2_Arm_L.rotateX')
        else:
            CreateNode('multDoubleLinear','MDL_'+Joint+'_00_Twist_L')
    ...
    CreateNode('multDoubleLinear','MDL_Hand_00_Twist_L')
    cmds.connectAttr('Loc_Extract_Hand_L.rotateX','MDL_Hand_00_Twist_L.input1')
    cmds.connectAttr('J_Hand_Neutral_L.rotate','Hand_Twist_L.rotate')    
    cmds.setAttr('MDL_Hand_00_Twist_L.input2',.5)
    cmds.connectAttr('MDL_Hand_00_Twist_L.output','Z_J_Bendy_2_ForeArm_L.rotateX')
    cmds.group(n='Twists_Arm_L',em=True)
    cmds.select(cl=True)
    cmds.parent('Twists_Arm_L','hidden')
    cmds.select(cl=True)
    Twist_Arm=['Arm','ForeArm','Hand']
    for twist in Twist_Arm:
        cmds.parent('Twist_'+twist+'_L','Twists_Arm_L')
        cmds.select(cl=True)
    cmds.hide('Twists_Arm_L')