...
def Twist_Arm_R():
    ...
    num=1
    List=['J_Arm_Neutral_R','J_ForeArm_Neutral_R','J_Hand_Neutral_R']
    for J in List:
    ...
    cmds.move(2*Scale_Guide,0,0,'Z_Hand_Eff_Twist_R',r=True,os=True,wd=True)
    Joints=['Arm','ForeArm']
    for Joint in Joints:
        if Joint=='ForeArm':
            CreateNode('multDoubleLinear','MDL_'+Joint+'_00_Twist_R')
            cmds.connectAttr('Loc_Extract_'+Joint+'_R.rotate','J_Bendy_3_Arm_R.rotate')
            ...
            cmds.connectAttr('MDL_'+Joint+'_01_Twist_R.output','J_Bendy_2_Arm_R.rotateX')
        else:
            CreateNode('multDoubleLinear','MDL_'+Joint+'_00_Twist_R')
    ...
    CreateNode('multDoubleLinear','MDL_Hand_00_Twist_R')
    cmds.connectAttr('Loc_Extract_Hand_R.rotateX','MDL_Hand_00_Twist_R.input1')
    cmds.connectAttr('J_Hand_Neutral_R.rotate','Hand_Twist_R.rotate')    
    cmds.setAttr('MDL_Hand_00_Twist_R.input2',.5)
    cmds.connectAttr('MDL_Hand_00_Twist_R.output','Z_J_Bendy_2_ForeArm_R.rotateX')
    cmds.group(n='Twists_Arm_R',em=True)
    cmds.select(cl=True)
    cmds.parent('Twists_Arm_R','hidden')
    cmds.select(cl=True)
    Twist_Arm=['Arm','ForeArm','Hand']
    for twist in Twist_Arm:
        cmds.parent('Twist_'+twist+'_R','Twists_Arm_R')
        cmds.select(cl=True)
    cmds.hide('Twists_Arm_R')