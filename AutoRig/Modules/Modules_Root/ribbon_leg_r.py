...
def Ribbon_Leg_R():
    ...
    Create_Nurb_Rib_Leg_R= cmds.nurbsPlane(n="Rib_Leg_R",p=[0,0,0],ax=[0,0,-1],w=6,lr=0.167,d=3,u=6,v=1,ch=1)#CREAR PLANO_Leg
    Get_Trans_Rot('J_Leg_Neutral_R')
    Set_Trans_Rot(Create_Nurb_Rib_Leg_R)
    cmds.parent("Rib_Leg_R","J_Leg_Neutral_R")
    cmds.rename('makeNurbPlane1', 'makeRib_Leg_R')
    Extract_TransX_ForeLeg_R = cmds.getAttr ("J_ForeLeg_Neutral_R.translateX")
    Divide_TransX_ForeLeg_R= ((Extract_TransX_ForeLeg_R)/2)
    Move_TransX_RibLeg_R = cmds.setAttr ("Rib_Leg_R.translateX",Divide_TransX_ForeLeg_R)
    Scale_Nurb_Rib_Leg_R= cmds.setAttr ("makeRib_Leg_R.width", Extract_TransX_ForeLeg_R)      
    Create_Nurb_Rib_ForeLeg_R= cmds.nurbsPlane(n="Rib_ForeLeg_R",p=[0,0,0],ax=[0,0,-1],w=6,lr=0.167,d=3,u=6,v=1,ch=1)#CREAR PLANO_FORELeg
    Get_Trans_Rot('J_ForeLeg_Neutral_R')
    Set_Trans_Rot(Create_Nurb_Rib_ForeLeg_R)
    Parent_J_ForeLeg_Neutral_R=cmds.parent("Rib_ForeLeg_R","J_ForeLeg_Neutral_R")
    Rename_MakeNurb=cmds.rename('makeNurbPlane1', 'makeRib_ForeLeg_R')
    Extract_TransX_Ankle_R = cmds.getAttr ("J_Ankle_Neutral_R.translateX")
    Divide_TransX_Ankle_R= ((Extract_TransX_Ankle_R)/2)
    Move_TransX_RibForeLeg_R = cmds.setAttr ("Rib_ForeLeg_R.translateX",Divide_TransX_Ankle_R)
    Scale_Nurb_Rib_ForeLeg_R= cmds.setAttr ("makeRib_ForeLeg_R.width", Extract_TransX_Ankle_R)
    cmds.parent("Rib_Leg_R",w=True)
    cmds.parent("Rib_ForeLeg_R",w=True) 
    cmds.select(cl=True)
    List_Pos_J_Legs=['J_Leg_Neutral_R','J_ForeLeg_Neutral_R']
    Joints=('J_ForeLeg_Neutral_R')
    for J_1 in List_Pos_J_Legs:#FOR PARA POSICIONAR Y ORIENTAR LOS CTRLS BENDY
        ...
        cmds.setAttr ('P_R_Bendy'+New_Name+'_02_CTL.translateX',Divide_TransX_ForeLeg_R)
        ...
        cmds.hide('Z_J_Bendy_3_'+New_Name+'_R')    
        ...
        Joints=('J_Ankle_Neutral_R')
        Divide_TransX_ForeLeg_R=Divide_TransX_Ankle_R
    Get_Trans_Rot('J_ForeLeg_Neutral_R')
    Set_Trans_Rot('P_R_BendyMasterLeg_CTL')
    Skn_Leg_Rib_R=cmds.skinCluster('J_Bendy_1_Leg_R', 'J_Bendy_2_Leg_R','J_Bendy_3_Leg_R', 'Rib_Leg_R',dr=4,tsb=True,mi=4)[0]
    Skn_For_ForeLeg_R=cmds.skinCluster('J_Bendy_1_ForeLeg_R','J_Bendy_2_ForeLeg_R','J_Bendy_3_ForeLeg_R', 'Rib_ForeLeg_R',dr=4,tsb=True,mi=4)[0]
    List_J_Bendys_Leg=[Skn_Leg_Rib_R,Skn_For_ForeLeg_R]#LISTA DE LOS NOMBRES DE LOS SKINS DE LOS PLANOS
    Name_Nurb='Rib_Leg_R'
    Joint_Skin='Leg_'
    for Skn in List_J_Bendys_Leg:#FOR PARA LIMPIAR LOS PESOS DEL SKIN DE LOS PLANOS
        ...
        Name_Nurb='Rib_ForeLeg_R'
        Joint_Skin='ForeLeg_'
    Point_Cons('R_BendyMasterLeg_CTL','Z_R_BendyLeg_03_CTL')
    Point_Cons('R_BendyMasterLeg_CTL','Z_R_BendyForeLeg_01_CTL')
    Point_Cons('R_BendyMasterLeg_CTL','Z_R_BendyForeLeg_02_CTL')
    Point_Cons('R_BendyMasterLeg_CTL','Z_R_BendyLeg_02_CTL')
    Point_Cons('R_BendyForeLeg_03_CTL','Z_R_BendyForeLeg_02_CTL')
    Point_Cons('R_BendyLeg_01_CTL','Z_R_BendyLeg_02_CTL')
    XYZ=['X','Y','Z']
    Ctrls_Bendy=['R_BendyForeLeg_01_CTL','R_BendyLeg_03_CTL']
    for Ctrl in Ctrls_Bendy: #FOR PARA CONECTAR LAS ROTATICIONES DEL CTR_BENDY_MASTER A LOS CONTROLES 1_FORELeg Y 3_Leg
        for xyz in XYZ:
            cmds.connectAttr('R_BendyMasterLeg_CTL.rotate'+xyz,Ctrl+'.rotate'+xyz, f=True)
    cmds.hide('R_BendyLeg_03_CTL','R_BendyLeg_01_CTL','R_BendyForeLeg_01_CTL','R_BendyForeLeg_03_CTL','R_BendyForeLeg_01_CTL')
    ...
    for Ctrl in Joints:  
        ...
        cmds.aimConstraint('R_BendyMasterLeg_CTL','Z_R_Bendy'+Ctrl+'_0'+Num_Ctrl_Children_2+'_CTL',mo=True,aim=[Valor_Neg,0,0],u=[1,0,0],wu=[0,1,0],wut="objectrotation",worldUpObject='R_BendyMasterLeg_CTL')
        cmds.aimConstraint('R_BendyMasterLeg_CTL','Z_R_Bendy'+Ctrl+'_0'+Num_Ctrl_Children_3+'_CTL',mo=True,aim=[Valor_Neg,0,0],u=[1,0,0],wu=[0,1,0],wut="objectrotation",worldUpObject='R_BendyMasterLeg_CTL') 
        Valor_Pos=-1
        Valor_Neg=1
        Num_Ctrl_Father='3'
   ...
   #FOR PARA CONTRAINTS AIMS
    cmds.group(n='Z_Ribs_Nurb_Leg_R',em=True)
    cmds.parent('Rib_ForeLeg_R','Z_Ribs_Nurb_Leg_R')
    cmds.parent('Rib_Leg_R','Z_Ribs_Nurb_Leg_R')
    cmds.select(cl=True) 
    cmds.select('Rib_Leg_R','Rib_ForeLeg_R')
    ...
    for nurb in range(lsel):#FOR PARA LOS RIVETS DE LOS RIBBON
        ...
        for Loc in range(Gett_Spans_Nurb):
            ...
            print Repeticiones   
    cmds.select(d=True)
    cmds.connectAttr('J_Leg_Neutral_R.scale','J_Bendy_2_Leg_R.scale')
    cmds.connectAttr('J_ForeLeg_Neutral_R.scale','J_Bendy_2_ForeLeg_R.scale')
    cmds.parentConstraint('J_ForeLeg_Neutral_R','P_R_BendyMasterLeg_CTL',mo=True)
    cmds.parentConstraint('J_ForeLeg_Neutral_R','P_R_BendyForeLeg_01_CTL',mo=True)
    cmds.parentConstraint('J_Ankle_Neutral_R','P_R_BendyForeLeg_03_CTL',mo=True)
    cmds.parentConstraint('J_Leg_Neutral_R','P_R_BendyLeg_01_CTL',mo=True)
    List=['Leg','ForeLeg']
    for J in List:
